...
function setupFileSaveListener(context: vscode.ExtensionContext) {
    const disposable = vscode.workspace.onDidSaveTextDocument(async (document) => {
        if (document.languageId === 'python') {
            await analyzeFileChanges(document);
        }
    });
...
async function analyzeFileChanges(document: vscode.TextDocument) {
    try {
        const differences = await getGitDifferences(document);

        if (!differences) {
            console.log('No Git differences found or Git unavailable');
            return;
        }

        printDifferencesToConsole(differences, document);

        analyzeCopilotUsage(differences, document);
    } catch (error) {
        console.error('Error:', error);
    }
}

async function getGitDifferences(document: vscode.TextDocument) {
        ...
        return calculateDetailedDifferences(oldContent, newContent);
}
...
function analyzeCopilotUsage(differences: any, document: vscode.TextDocument) {
    const fileName = document.fileName.split('/').pop() || document.fileName;
